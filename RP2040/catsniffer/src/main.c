...
int apply_lora_config(void) {
    ...
    if(!catsniffer.lora_initialized) {
        status_msg = "Error: LoRa not initialized. Use TEST command first.\r\n";
        safe_ring_buf_put(&rb_config_to_usb, (uint8_t*)status_msg, strlen(status_msg));
        if(cdc2_dev) uart_irq_tx_enable(cdc2_dev);
        return -ENODEV;
    }
...
void process_lora_command(char* cmd_line) {
    ...
    if(strncmp(cmd_line, "TX ", 3) == 0) {
        ...
        snprintf(
            response, sizeof(response), "DEBUG: Hex input '%s', length %zu\r\n", hex_data, hex_len);
        safe_ring_buf_put(&rb_config_to_usb, (uint8_t*)response, strlen(response));
        if(cdc2_dev) uart_irq_tx_enable(cdc2_dev);
        ...
        if(hex_len % 2 == 0 && hex_len > 0) {
            ...
            snprintf(
                response, sizeof(response), "TX Result: %d (%s)\r\n", ret, get_error_string(ret));
