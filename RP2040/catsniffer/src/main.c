...
void process_lora_command(char *cmd_line)
{
	...
	if (strncmp(cmd_line, "TX ", 3) == 0) {
		...
		if (hex_len % 2 == 0 && hex_len > 0) {
			...
			for (size_t i = 0; i < data_len; i++) {
				snprintf(&debug_hex[i * 3], 4, "%02X ",
					 tx_data[i]);
			}
			snprintf(response, sizeof(response),
				 "DEBUG: Sending bytes %s\r\n", debug_hex);
			safe_ring_buf_put(&rb_config_to_usb,
					  (uint8_t *)response,
					  strlen(response));
			if (cdc2_dev)
	...
	while (1) {
		...
		if (catsniffer.lora_mode == LORA_MODE_COMMAND) {
			...
			int usb_len = safe_ring_buf_get(
				&rb_usb_to_sx1262, usb_buf, sizeof(usb_buf));
			...
			int tx_len = safe_ring_buf_get(&rb_usb_to_sx1262,
						       tx_buffer,
						       sizeof(tx_buffer));
...
int main(void)
{
	...
	k_thread_create(&lora_thread, lora_thread_stack, LORA_THREAD_STACK_SIZE,
			lora_thread_func, NULL, NULL, NULL, K_PRIO_COOP(5), 0,
			K_NO_WAIT);
