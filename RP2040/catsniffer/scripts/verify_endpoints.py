...
def print_device_info(devices):
    ...
    for dev in devices:
        print(f"\n{dev}")
        print(f"  \033[36mCat-Bridge (CC1352):\033[0m {dev.bridge_port or '\033[91mNot found\033[0m'}")
        print(f"  \033[36mCat-LoRa (SX1262):\033[0m   {dev.lora_port or '\033[91mNot found\033[0m'}")
        print(f"  \033[36mCat-Shell (Config):\033[0m  {dev.shell_port or '\033[91mNot found\033[0m'}")
...
def test_lora_config_commands(device):
    ...
    tests_total += 1
    response = device.send_command(device.shell_port, "lora_freq 915000000", timeout=2.0)
...
def test_lora_communication(device):
    ...
    tests_total += 1
    success, response = send_lora_listen_shell("TEST", ["TEST", "LoRa ready", "initialized"], timeout=4.0)
    if success:
    ...
    tests_total += 1
    success, response = send_lora_listen_shell("TEST", ["TEST", "LoRa ready"], timeout=4.0)
    if success:
    ...
    tests_total += 1
    success, response = send_lora_listen_shell("TXTEST", ["TX Result", "DEBUG: Sending", "Success"], timeout=5.0)
    if success:
    ...
    tests_total += 1
    success, response = send_lora_listen_shell("TX 50494E47", ["TX Result", "Success"], timeout=5.0)
    if success:
...
def main():
    ...
    for device_id, tests in results.items():
        ...
        basic_status = "\033[32m✓ PASS\033[0m" if tests['basic'] else "\033[31m✗ FAIL\033[0m"
        print(f"  \033[36mBasic Commands:\033[0m        {basic_status}")

        if args.test_all:
            config_status = "\033[32m✓ PASS\033[0m" if tests.get('config') else "\033[31m✗ FAIL\033[0m"
            lora_status = "\033[32m✓ PASS\033[0m" if tests.get('lora') else "\033[31m✗ FAIL\033[0m"
