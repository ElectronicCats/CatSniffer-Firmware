/*
  LoRaSniffer - Use LoRa for communication with the SX1262 module and
  PyCatSniffer PLEASE REFER TO THESE LIBRARIES:
...
int16_t floatToTwoBytesSigned(float value, float minValue, float maxValue)
{
	...
	int16_t intValue =
		(int16_t)((value - minValue) / (maxValue - minValue) * 32767);
	return intValue;
}
...
void set_rx_hop()
{
	...
	if (currentTime - lastHopTime >= hopInterval) {
		...
		currentHopChannelIdx =
			(currentHopChannelIdx + 1) % numHopChannels;
