...
void setup()
{
	...
	int state = radio.beginFSK(radioCtx.freqStart, radioCtx.biteRate,
				   radioCtx.freqDeviation, radioCtx.bandwidth,
				   radioCtx.power, 16, 0, false);
...
void loop()
{
	...
	while ((radioCtx.freq <= radioCtx.freqEnd) && runningScan) {
		...
		if (state == RADIOLIB_ERR_NONE) {
			...
			Serial.print("SCAN ");
			for (uint8_t i = 0;
			     i < RADIOLIB_SX126X_SPECTRAL_SCAN_RES_SIZE; i++) {
...
void cmdGetConfiguration()
{
	...
	Serial.println("Frequency Start= " + String(radioCtx.freqStart) +
		       " MHz");
...
void help()
{
	...
	Serial.println("Set the frequency start: Default " +
		       String(FREQ_RANGE_START));
	Serial.print("set_end_freq ");
	Serial.println("Set the frequency end: Default " +
