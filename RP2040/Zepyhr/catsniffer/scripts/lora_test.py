...
class LoRaTester:
    ...
    def send_shell_command(self, port: serial.Serial, cmd: str, timeout: float = 1.0) -> str:
    ...
    def configure_lora(self, frequency: int = 915000000, sf: int = 7,
                       bw: int = 125, cr: int = 5, power: int = 20):
    ...
    def run_ping_test(self, count: int = 10) -> dict:
        ...
        for i in range(count):
            ...
            try:
                ...
                if rx_payload[:4] == b'PING' and rx_payload[4:6] == seq:
                    ...
                    print(f"  [{i+1:3d}/{count}] OK  RSSI: {rssi:4d} dBm  SNR: {snr:3d} dB  RTT: {rtt:6.1f} ms")
                else:
        ...
        if rtts:
            print(f"  RTT:  {avg_rtt:.1f} ms avg, {min(rtts):.1f} ms min, {max(rtts):.1f} ms max")

        return results

    def run_throughput_test(self, packet_count: int = 50, payload_size: int = 50) -> dict:
        ...
        rx_throughput = (total_bytes_received * 8) / duration if duration > 0 else 0  # bps
        ...
        print(f"  TX Throughput: {tx_throughput:.1f} bps ({tx_throughput/1000:.2f} kbps)")
        print(f"  RX Throughput: {rx_throughput:.1f} bps ({rx_throughput/1000:.2f} kbps)")
    ...
    def run_sf_comparison(self, payload_size: int = 20, packets_per_sf: int = 10) -> dict:
        """Compare throughput across different spreading factors."""
        print(f"\n{'='*60}")
        print("SPREADING FACTOR COMPARISON")
        print(f"{'='*60}")
        print(f"Testing SF7-SF12 with {packets_per_sf} packets of {payload_size} bytes each\n")
        ...
        for sf in range(7, 13):
            key = f'SF{sf}'
            r = results[key]
            print(f"{key:<6} {r['loss_percent']:<10.1f} "
...
def find_all_catsniffers() -> List[CatSnifferDevice]:
    ...
    usb_devices = list(usb.core.find(
        find_all=True,
        idVendor=CATSNIFFER_VID,
        idProduct=CATSNIFFER_PID
    ))
        ...
        for cfg in dev:
            for intf in cfg:
                if intf.bInterfaceClass == 0x02:  # CDC Control
                    try:
                        name = usb.util.get_string(dev, intf.iInterface) if intf.iInterface else None
                    except:
                        name = None
    ...
    for device_idx in range(len(usb_devices)):
        ...
        if port_offset + 2 < len(cat_ports):
            ...
            devices.append(CatSnifferDevice(
...
def main():
    ...
    parser.add_argument('--tx', type=str, help='TX device shell port (e.g., /dev/cu.usbmodem2105)')
    parser.add_argument('--rx', type=str, help='RX device shell port (e.g., /dev/cu.usbmodem2205)')
    parser.add_argument('--packets', type=int, default=50, help='Number of packets for throughput test')
    ...
    parser.add_argument('--compare-sf', action='store_true', help='Run SF comparison test')
