...
void catsnifferUpdateLedsAnimation(void) {
    if(millis() - cs_context.previousMillis > cs_context.led_interval) {
        ...
        if(cs_context.mode) {
            digitalWrite(LEDs[ledsIdx], !digitalRead(LEDs[ledsIdx]));
            ledsIdx++;
            if(ledsIdx > 2) ledsIdx = 0;
        } else {
...
void catsnifferCommandProcess(void) {
    ...
    if(Serial.available()) {
        ...
        if(data == commandID[cmdCounter]) {
            cmdCounter++;
            if(cmdCounter == 5) cmdRecognized = 1;
