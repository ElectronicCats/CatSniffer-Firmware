#!/bin/bash
# Catsniffer v1/v2 (SAMD21) staged build/flash/test script
# Flashing uses the uf2-samdx1 bootloader: the board mounts as a USB drive
# containing INFO_UF2.TXT and the firmware is copied onto it.
...
Useful flags:
  --all Flash all detected CatSniffers (all mounted UF2 drives)
...
# A UF2 bootloader drive is any mount that carries INFO_UF2.TXT
list_mount_points() {
    local mp
    for mp in $(compgen -G "$MOUNT_GLOB" || true); do
...
wait_for_mount() {
    wait_for_any_mount || return 1
    MOUNT_POINT="$(list_mount_points | head -n 1)"
}
...
wait_for_any_mount() {
    ...
    while [[ -z "$(list_mount_points)" ]]; do
        ...
        if [[ $elapsed -ge $timeout ]]; then
            ...
            echo "       Try: double-tap the RESET button to enter the UF2 bootloader"
            return 1
        fi
...
if [[ $DO_COMPILE -eq 1 ]]; then
    ...
    # Run west from the workspace that owns ZEPHYR_BASE so the build
    # extension commands are available even when SAMD21/ has no workspace.
    WEST_DIR="$(dirname "$ZEPHYR_BASE")"
    BUILD_BASE=(west build -s "$PROJECT_DIR" -d "$PROJECT_DIR/build" -b catsniffer_v2)
    ...
    if [[ $PRISTINE_BUILD -eq 1 ]]; then
        ...
        (cd "$WEST_DIR" && "${BUILD_BASE[@]}" -p always -- "${CMAKE_ARGS[@]}")
    else
        if [[ -d "$PROJECT_DIR/build" && ! -f "$PROJECT_DIR/build/build.ninja" ]]; then
            ...
            (cd "$WEST_DIR" && "${BUILD_BASE[@]}" -- "${CMAKE_ARGS[@]}")
        fi
    fi
...
if [[ $DO_FLASH -eq 1 ]]; then
    ...
    if [[ $ALL_DEVICES -eq 1 ]]; then
        ...
        echo "[flash] Waiting for any UF2 bootloader drive..."
        wait_for_any_mount
        ...
        echo "[flash] Waiting for a UF2 bootloader drive..."
        wait_for_mount
