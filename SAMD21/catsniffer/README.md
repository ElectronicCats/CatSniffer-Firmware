...
- CC1352P on SERCOM0 UART (PA04 TX, PA05 RX), 921600 baud
...
- UF2 bootloader (uf2-samdx1) in the first 8 KB, application at 0x2000
...
- Buffers are small to fit 16 KB SRAM: Cat-Bridge 512 B per direction
  (`CONFIG_CATSNIFFER_BRIDGE_RING_SIZE`), LoRa and shell 256 B, LoRa thread
  stack 1 KB. `status` shows UART overrun and ring overflow counters; use
  them to judge whether the bridge keeps up with your CC1352 firmware.
- Logging is off. `debug.conf` adds printk and the thread analyzer over
  Segger RTT (SWD) with the bridge buffers reduced to 256 B.

## Build

Requires the same toolchain as the RP2040 firmware: Zephyr SDK 0.17.0, the
`wero1414/zephyr` fork on branch `fsk-native-driver` (native SX126x driver),
and west in `~/zephyrproject/.venv`.
...
# Debug variant (RTT console, thread analyzer)
...
Not yet. See `docs/superpowers/plans/2026-04-13-samd21-catsniffer-v2.md`,
Task 7, for the checklist.
