...
#define DPRINTF(...) Display_printf(dispHandle, DISPLAY_ROW_UNUSED, 0, __VA_ARGS__)
...
#define JW_ADV_RPT_FIELDS ( SCAN_ADVRPT_FLD_EVENTTYPE  | \
...
#define JW_ASSERT(expr) if (!(expr)) JustWorksScanner_spin()

/* ========================= Types =========================== */

typedef struct { appEvtHdr_t hdr; uint8_t *pData; } jwEvt_t;
...
typedef struct { uint16_t connHandle; uint8_t status; } jwPairStateData_t;
...
typedef struct { uint8_t addr[B_ADDR_LEN]; char name[31]; int8 lastRssi; uint32_t lastLogMs; } nameCache_t;
#define NAME_CACHE_MAX 48
static nameCache_t nameCache[NAME_CACHE_MAX];

/* Per-MAC connect throttle */
typedef struct { uint8_t addr[B_ADDR_LEN]; uint32_t tick; uint8_t attempts; uint8_t exhausted; } connectTry_t;
...
static void JustWorksScanner_logInsecureJustWorks(uint16_t connHandle, const char* reason);
...
/* BondMgr callbacks */
static void JustWorksScanner_passcodeCb(uint8_t *deviceAddr, uint16_t connHandle,
                                        uint8_t uiInputs, uint8_t uiOutputs,
                                        uint32_t numComparison);
static void JustWorksScanner_pairStateCb(uint16_t connHandle, uint8_t state, uint8_t status);

/* Device name discovery / write */
static void startDevNameDiscovery(uint16_t connHandle);
static void handleReadByGrpTypeRsp(uint16_t connHandle, const attReadByGrpTypeRsp_t *rsp);
static void handleReadByTypeRsp_Name(uint16_t connHandle, const attReadByTypeRsp_t *rsp);
static void continueNameFlow(uint16_t connHandle);

/* ========================= Small helpers =================== */

static bool isZeroBuf(const uint8_t *buf, size_t len)
{ for (size_t i=0;i<len;i++){ if (buf[i]) return false; } return true; }
...
static void cacheNameRssi(uint8_t addr[B_ADDR_LEN], const char* name, int8 rssi)
{
  ...
  for (i=0;i<NAME_CACHE_MAX;i++){
    if (memcmp(nameCache[i].addr, addr, B_ADDR_LEN)==0){
      if (name){ strncpy(nameCache[i].name, name, sizeof(nameCache[i].name)-1); nameCache[i].name[sizeof(nameCache[i].name)-1]=0; }
      nameCache[i].lastRssi = rssi; return;
    }
    if (freeIdx==0xFF && isZeroBuf(nameCache[i].addr, B_ADDR_LEN)) freeIdx=i;
  }
  if (freeIdx!=0xFF){
    memcpy(nameCache[freeIdx].addr, addr, B_ADDR_LEN);
    nameCache[freeIdx].lastRssi = rssi;
    if (name){ strncpy(nameCache[freeIdx].name, name, sizeof(nameCache[freeIdx].name)-1); nameCache[freeIdx].name[sizeof(nameCache[freeIdx].name)-1]=0; }
    else nameCache[freeIdx].name[0]=0;
...
static const char* lookupName(uint8_t addr[B_ADDR_LEN])
{
  for (uint8_t i=0;i<NAME_CACHE_MAX;i++)
    if (memcmp(nameCache[i].addr, addr, B_ADDR_LEN)==0)
      return nameCache[i].name[0]?nameCache[i].name:"(no-name)";
  return "(no-name)";
}
...
static bool logAllowed(uint8_t addr[B_ADDR_LEN])
{
  ...
  for (uint8_t i=0;i<NAME_CACHE_MAX;i++){
    if (memcmp(nameCache[i].addr, addr, B_ADDR_LEN)==0){
      if (now - nameCache[i].lastLogMs >= LOG_RATELIMIT_MS) { nameCache[i].lastLogMs = now; return true; }
      return false;
    }
  }
  return true;
...
static bool connectTry_allowedAndMark(uint8_t addr[B_ADDR_LEN])
{
  ...
  for (int i=0;i<CONNECT_TRY_MAX;i++){
    if (isZeroBuf(connectTries[i].addr, B_ADDR_LEN)) { if (freeIdx<0) freeIdx=i; continue; }
    if (memcmp(connectTries[i].addr, addr, B_ADDR_LEN)==0){
      if (connectTries[i].exhausted) return false;
      if ((now - connectTries[i].tick) < CONNECT_RETRY_BACKOFF_MS) return false;
      reuseIdx = i; break;
    }
  }
  ...
  if (connectTries[idx].attempts < 0xFF) connectTries[idx].attempts++;
...
static uint8_t parseLocalName(uint8_t *pData, uint16_t len, char *out, uint8_t outSz)
{
  uint8_t *p=pData, *end=pData+len;
  while (p+1<end){
    uint8_t adLen=*p++; if (adLen==0 || p+adLen>end) break;
    uint8_t adType=*p++; uint8_t fieldLen=adLen-1;
    if (adType==GAP_ADTYPE_LOCAL_NAME_COMPLETE || adType==GAP_ADTYPE_LOCAL_NAME_SHORT){
      uint8_t cpy = (fieldLen<(outSz-1))?fieldLen:(outSz-1);
      memcpy(out,p,cpy); out[cpy]=0; return 1;
    }
    p+=fieldLen;
  }
  return 0;
...
{
  Task_Params taskParams; Task_Params_init(&taskParams);
  taskParams.stack = jwTaskStack; taskParams.stackSize = JW_TASK_STACK_SIZE; taskParams.priority = JW_TASK_PRIORITY;
  Task_construct(&jwTask, JustWorksScanner_taskFxn, &taskParams, NULL);
}

/* ========================= Init/Task ======================= */

static void JustWorksScanner_spin(void){ volatile uint8_t x=0; while(1){ x++; } }
...
static void JustWorksScanner_init(void)
{
  ...
  for (uint8_t i=0;i<MAX_NUM_BLE_CONNS;i++) connList[i].connHandle = LINKDB_CONNHANDLE_INVALID;
  ...
  static gapBondCBs_t bondMgrCBs = { JustWorksScanner_passcodeCb, JustWorksScanner_pairStateCb };
...
static void JustWorksScanner_taskFxn(uintptr_t a0, uintptr_t a1)
{
  ...
  for (;;)
  {
    uint32_t events = Event_pend(syncEvent, Event_Id_NONE, JW_ALL_EVENTS, ICALL_TIMEOUT_FOREVER);

    if (events)
    {
      ICall_EntityID dest; ICall_ServiceEnum src; ICall_HciExtEvt *pMsg = NULL;
      if (ICall_fetchServiceMsg(&src, &dest, (void **)&pMsg) == ICALL_ERRNO_SUCCESS)
      {
        ...
        if ((src == ICALL_SERVICE_CLASS_BLE) && (dest == selfEntity))
        {
          ...
          if (pEvt->signature != 0xFFFF) safeToDealloc = JustWorksScanner_processStackMsg((ICall_Hdr*)pMsg);
        }
        if (pMsg && safeToDealloc) ICall_freeMsg(pMsg);
      }

      if (events & JW_QUEUE_EVT)
      {
        jwEvt_t *pMsg;
        while ((pMsg = (jwEvt_t*)Util_dequeueMsg(appMsgQueue)))
        {
...
static uint8_t JustWorksScanner_processStackMsg(ICall_Hdr *pMsg)
{
  switch (pMsg->event)
  {
    case GAP_MSG_EVENT:  JustWorksScanner_processGapMsg((gapEventHdr_t*)pMsg); break;
    case GATT_MSG_EVENT: JustWorksScanner_processGATTMsg((gattMsgEvent_t*)pMsg); break;
...
static status_t JustWorksScanner_enqueueMsg(uint8_t event, uint8_t state, uint8_t *pData)
{
  jwEvt_t *pMsg = ICall_malloc(sizeof(jwEvt_t));
  if (pMsg)
  {
    pMsg->hdr.event = event; pMsg->hdr.state = state; pMsg->pData = pData;
    if (Util_enqueueMsg(appMsgQueue, syncEvent, (uint8_t *)pMsg)) return SUCCESS;
    ICall_free(pMsg);
  }
...
static void JustWorksScanner_processAppMsg(jwEvt_t *pMsg)
{
  switch (pMsg->hdr.event)
  {
    ...
    {
      GapScan_Evt_AdvRpt_t* pAdv = (GapScan_Evt_AdvRpt_t*)(pMsg->pData);
      char name[31]={0}; parseLocalName(pAdv->pData, pAdv->dataLen, name, sizeof(name));
      cacheNameRssi(pAdv->addr, name[0]?name:NULL, pAdv->rssi);

      if (logAllowed(pAdv->addr))
      {
        char evtStr[64]; evtTypeToStr(pAdv->evtType, evtStr, sizeof(evtStr));
        DPRINTF("[SCAN] ADV from %s | name:%s | RSSI:%d dBm | addrType:%s | primPHY:%s secPHY:%s evt:%s",
                ...
                addrTypeStr(pAdv->addrType), phyToStr(pAdv->primPhy), phyToStr(pAdv->secPhy), evtStr);
      }

      /* Connect to connectable devices (dedupe/backoff/limit) */
      if (numConn < MAX_NUM_BLE_CONNS && (pAdv->evtType & ADV_RPT_EVT_TYPE_CONNECTABLE))
      {
        bool exhausted = false;
        for (int i=0;i<CONNECT_TRY_MAX;i++){
          if (memcmp(connectTries[i].addr, pAdv->addr, B_ADDR_LEN)==0 && connectTries[i].exhausted){ exhausted=true; break; }
        }
        if (!exhausted && connectTry_allowedAndMark(pAdv->addr))
        {
          DPRINTF("[CONN] Initiating to %s ...", Util_convertBdAddr2Str(pAdv->addr));
          GapInit_connect(pAdv->addrType & MASK_ADDRTYPE_ID, pAdv->addr, DEFAULT_INIT_PHY, CONNECTION_TIMEOUT);
        }
        else if (exhausted)
        {
          DPRINTF("[CONN] Max attempts reached for %s; skipping further tests", Util_convertBdAddr2Str(pAdv->addr));
        }
      }

      if (pAdv->pData) ICall_free(pAdv->pData);
    } break;
    ...
    case JW_EVT_SCAN_RESTART:
      DPRINTF("[SCAN] restart tick (every %ums)", (unsigned)SCAN_RESTART_PERIOD);
      switch (scanState) {
        ...
        case SCAN_STATE_DISABLING:
          ...
          DPRINTF("[SCAN] transition in progress (%s); will restart after it completes",
                  scanState == SCAN_STATE_ENABLING ? "ENABLING" : "DISABLING");
          break;
      }
      break;
    ...
    {
      uint32_t now = msTicks();
      if ((scanState == SCAN_STATE_ENABLING || scanState == SCAN_STATE_DISABLING) &&
          (now - lastScanTransitionMs) > SCAN_TRANSITION_TIMEOUT_MS)
      {
        DPRINTF("[SCAN][WD] transition timeout in state=%d; forcing disable→enable", (int)scanState);
    ...
    case JW_EVT_PAIR_STATE:
    {
      ...
      else if (pMsg->hdr.state == GAPBOND_PAIRING_STATE_COMPLETE)
      {
        if (status == SUCCESS)
        {
          ...
          if (idx < MAX_NUM_BLE_CONNS && !sawPasscodePrompt[idx])
            JustWorksScanner_logInsecureJustWorks(ch, "no user interaction during pairing");
        }
        else
        {
          DPRINTF("[PAIR] failed (0x%02X %s) conn=0x%04X", status, pairStatusToStr(status), ch);
        }
      }
      else if (pMsg->hdr.state == GAPBOND_PAIRING_STATE_ENCRYPTED)
      {
        DPRINTF("[PAIR] %s (conn=0x%04X)", (status==SUCCESS)?"link encrypted":"encryption failed", ch);
      }
      else if (pMsg->hdr.state == GAPBOND_PAIRING_STATE_BOND_SAVED)
      {
        DPRINTF("[PAIR] bond %s (conn=0x%04X)", (status==SUCCESS)?"saved":"save failed", ch);
      }
    } break;
    ...
    case JW_EVT_PASSCODE_NEEDED:
    {
      ...
      DPRINTF("[PAIR] PasscodeReq conn=0x%04X uiIn:%u uiOut:%u numCmp:%lu",
              ch, pData->uiInputs, pData->uiOutputs, (unsigned long)pData->numComparison);

      uint8_t idx = JustWorksScanner_getConnIndex(ch);
      if (idx < MAX_NUM_BLE_CONNS) sawPasscodePrompt[idx] = true;

      if (pData->uiInputs==0 && pData->uiOutputs==0 && pData->numComparison==0)
      {
        JustWorksScanner_logInsecureJustWorks(ch, "no IO, no Numeric Comparison");
      }
    ...
    case JW_EVT_READ_RPA:
    {
      ...
      if (memcmp(pRpaNew, rpa, B_ADDR_LEN)) {
        DPRINTF("[DBG] RP Addr changed: %s", Util_convertBdAddr2Str(pRpaNew));
  ...
  if (pMsg->pData) ICall_free(pMsg->pData);
}
...
static void JustWorksScanner_processGapMsg(gapEventHdr_t *pMsg)
{
  switch (pMsg->opcode)
  {
    case GAP_DEVICE_INIT_DONE_EVENT:
    {
      ...
      GapScan_setPhyParams(DEFAULT_SCAN_PHY, DEFAULT_SCAN_TYPE, DEFAULT_SCAN_INTERVAL, DEFAULT_SCAN_WINDOW);

      temp16 = JW_ADV_RPT_FIELDS;         GapScan_setParam(SCAN_PARAM_RPT_FIELDS, &temp16);
      temp8  = DEFAULT_SCAN_PHY;          GapScan_setParam(SCAN_PARAM_PRIM_PHYS, &temp8);
      temp8  = SCANNER_DUPLICATE_FILTER;  GapScan_setParam(SCAN_PARAM_FLT_DUP, &temp8);
      ...
      DPRINTF("[INIT] Done. DevAddr: %s", Util_convertBdAddr2Str(pPkt->devAddr));

      if (addrMode > ADDRMODE_RANDOM)
      {
        memcpy(rpa, GAP_GetDevAddress(FALSE), B_ADDR_LEN);
        DPRINTF("[INIT] RP Addr: %s", Util_convertBdAddr2Str(rpa));
        Util_constructClock(&clkRpaRead, JustWorksScanner_clockHandler, 30000, 0, true, JW_EVT_READ_RPA);
      }
      ...
      Util_constructClock(&clkScanRestart,  JustWorksScanner_clockHandler, SCAN_RESTART_PERIOD, 0, true, JW_EVT_SCAN_RESTART);
      Util_constructClock(&clkScanWatchdog, JustWorksScanner_clockHandler, SCAN_WATCHDOG_PERIOD, 0, true, JW_EVT_SCAN_WATCHDOG);
      DPRINTF("[SCAN] periodic relaunch set to %ums", (unsigned)SCAN_RESTART_PERIOD);
    } break;
    ...
    case GAP_LINK_ESTABLISHED_EVENT:
    {
      ...
      uint8_t connIndex = JustWorksScanner_addConnInfo(e->connectionHandle, e->devAddr);
      JW_ASSERT(connIndex < MAX_NUM_BLE_CONNS);
      sawPasscodePrompt[connIndex] = false;

      DPRINTF("[CONN] Connected: %s | name:%s | RSSI:%d",
              Util_convertBdAddr2Str(e->devAddr), lookupName(e->devAddr), (int)lookupRssi(e->devAddr));
    ...
    case GAP_LINK_TERMINATED_EVENT:
    {
      ...
      uint8_t idx = JustWorksScanner_removeConnInfo(e->connectionHandle);
      JW_ASSERT(idx < MAX_NUM_BLE_CONNS);
      DPRINTF("[CONN] Disconnected: %s (reason=0x%02X %s)",
              Util_convertBdAddr2Str(connList[idx].addr), e->reason, discReasonToStr(e->reason));
    } break;

    case GAP_LINK_PARAM_UPDATE_EVENT:
    {
      gapLinkUpdateEvent_t *e = (gapLinkUpdateEvent_t*)pMsg; linkDBInfo_t info;
      if (linkDB_GetInfo(e->connectionHandle, &info) == SUCCESS)
        DPRINTF("[CONN] Param update: %s timeout:%dms",
                Util_convertBdAddr2Str(info.addr), info.connTimeout*CONN_TIMEOUT_MS_CONVERSION);
    } break;
...
static void JustWorksScanner_processGATTMsg(gattMsgEvent_t *pMsg)
{
  ...
  if (!linkDB_Up(ch)){ GATT_bm_free(&pMsg->msg, pMsg->method); return; }
  if (pMsg->hdr.status == blePending) DPRINTF("[GATT] ATT rsp dropped %d", pMsg->method);
  else if (pMsg->method == ATT_MTU_UPDATED_EVENT) DPRINTF("[GATT] MTU updated: %d", pMsg->msg.mtuEvt.MTU);

  nameDisc_t *c = NULL;
  for (uint8_t i=0;i<MAX_NUM_BLE_CONNS;i++)
    if (nameCtx[i].inUse && nameCtx[i].connHandle == ch){ c = &nameCtx[i]; break; }
    ...
    switch (pMsg->method)
    {
      ...
      case ATT_READ_BY_TYPE_RSP:
        if (c->step == DISC_READ_DEVNAME)
          handleReadByTypeRsp_Name(ch, &pMsg->msg.readByTypeRsp);
        break;

      case ATT_WRITE_RSP:
        if (c->step == DISC_WRITE_DEVNAME)
        {
          DPRINTF("[INFO] Name write OK: \"%s\" -> \"Secure your device\"", c->prevName);
          c->step = DISC_DONE;
        }
        break;
      ...
      case ATT_ERROR_RSP:
      {
        ...
        if (c->step == DISC_READ_DEVNAME)
        {
          DPRINTF("[INFO] GAP info flow stopped: DevName read failed (err=0x%02X, attr=0x%04X)", e->errCode, e->handle);
          c->step = DISC_DONE;
        }
        else if (c->step == DISC_WRITE_DEVNAME)
        {
          DPRINTF("[INFO] Name write failed (err=0x%02X)", e->errCode);
          c->step = DISC_DONE;
        }
      } break;
...
static void freeNameCtx(uint16_t connHandle)
{
  for (uint8_t i=0;i<MAX_NUM_BLE_CONNS;i++)
    if (nameCtx[i].inUse && nameCtx[i].connHandle == connHandle)
      { memset(&nameCtx[i], 0, sizeof(nameCtx[i])); return; }
}

static void startDevNameDiscovery(uint16_t connHandle)
{
  nameDisc_t *c = allocNameCtx(connHandle);
  if (!c) { DPRINTF("[INFO] GAP info flow skipped (no ctx)"); return; }

  /* 1) Discover GAP primary service (UUID 0x1800) */
  static const uint8 uuid_gap_1800[ATT_BT_UUID_SIZE] = { LO_UINT16(0x1800), HI_UINT16(0x1800) };

  bStatus_t s = GATT_DiscPrimaryServiceByUUID(connHandle, (uint8*)uuid_gap_1800, ATT_BT_UUID_SIZE, selfEntity);
  if (s != SUCCESS)
  {
    DPRINTF("[INFO] GAP info flow stopped: GAP service discover start failed (0x%02X)", s);
...
static void handleReadByGrpTypeRsp(uint16_t connHandle, const attReadByGrpTypeRsp_t *rsp)
{
  ...
  DPRINTF("[INFO] GAP svc [0x%04X..0x%04X]", c->gapStartHdl, c->gapEndHdl);

  /* 2) Read Device Name using UUID filter (0x2A00) */
  static const uint8 devNameUUID[ATT_BT_UUID_SIZE] = { LO_UINT16(0x2A00), HI_UINT16(0x2A00) };

  attReadByTypeReq_t r; memset(&r, 0, sizeof(r));
  ...
  if (s != SUCCESS)
  {
    DPRINTF("[INFO] GAP info flow stopped: DevName read start failed (0x%02X)", s);
    c->step = DISC_DONE;
    return;
  }
...
static void handleReadByTypeRsp_Name(uint16_t connHandle, const attReadByTypeRsp_t *rsp)
{
  ...
  uint8_t copyLen = (valueLen < sizeof(c->prevName)-1) ? valueLen : (sizeof(c->prevName)-1);
  memcpy(c->prevName, p+2, copyLen);
  c->prevName[copyLen] = 0;

  DPRINTF("[INFO] Device Name: \"%s\"", c->prevName[0]?c->prevName:"(empty)");
...
static void continueNameFlow(uint16_t connHandle)
{
  ...
  if (c->step == DISC_WRITE_DEVNAME && c->devNameValHdl != 0)
  {
    ...
    uint8_t len = (uint8_t)MIN(sizeof(kNewName)-1, ATT_MTU_SIZE-3);

    attWriteReq_t w; memset(&w, 0, sizeof(w));
    ...
    if (s != SUCCESS)
    {
      DPRINTF("[INFO] Name write failed to start (0x%02X)", s);
      ICall_free(w.pValue);
      c->step = DISC_DONE;
    }
    else
                                        ...
                                        uint32_t numComparison)
{
  ...
  if (pData)
  {
    ...
    if (JustWorksScanner_enqueueMsg(JW_EVT_PASSCODE_NEEDED, 0, (uint8_t*)pData) != SUCCESS) ICall_free(pData);
  }
}
...
static void JustWorksScanner_pairStateCb(uint16_t connHandle, uint8_t state, uint8_t status)
{
  ...
  if (p)
  {
    p->connHandle = connHandle; p->status = status;
    if (JustWorksScanner_enqueueMsg(JW_EVT_PAIR_STATE, state, (uint8_t*)p) != SUCCESS) ICall_free(p);
  }
}
...
{
  uint8_t event;
  if (evt & GAP_EVT_ADV_REPORT)               event = JW_EVT_ADV_REPORT;
  else if (evt & GAP_EVT_SCAN_ENABLED)        event = JW_EVT_SCAN_ENABLED;
  else if (evt & GAP_EVT_SCAN_DISABLED)       event = JW_EVT_SCAN_DISABLED;
  else if (evt & GAP_EVT_INSUFFICIENT_MEMORY) event = JW_EVT_INSUFFICIENT_MEM;
  else return;

  if (JustWorksScanner_enqueueMsg(event, SUCCESS, pMsg) != SUCCESS) ICall_free(pMsg);
}
...
void JustWorksScanner_clockHandler(UArg arg)
{
  ...
  switch (evtId)
  {
    case JW_EVT_READ_RPA:      Util_startClock(&clkRpaRead);      JustWorksScanner_enqueueMsg(JW_EVT_READ_RPA, 0, NULL);      break;
    case JW_EVT_SCAN_RESTART:  Util_startClock(&clkScanRestart);  JustWorksScanner_enqueueMsg(JW_EVT_SCAN_RESTART, 0, NULL);  break;
    case JW_EVT_SCAN_WATCHDOG: Util_startClock(&clkScanWatchdog); JustWorksScanner_enqueueMsg(JW_EVT_SCAN_WATCHDOG, 0, NULL); break;
    default: break;
  }
}
...
{
  for (uint8_t i=0;i<MAX_NUM_BLE_CONNS;i++)
    if (connList[i].connHandle == connHandle) return i;
  return MAX_NUM_BLE_CONNS;
}
#ifndef Display_DISABLE_ALL
static char* JustWorksScanner_getConnAddrStr(uint16_t connHandle)
{
  for (uint8_t i=0;i<MAX_NUM_BLE_CONNS;i++)
    if (connList[i].connHandle == connHandle) return Util_convertBdAddr2Str(connList[i].addr);
  return NULL;
}
#endif
...
static void setBondManagerParameters_ForJustWorks(void)
{
  ...
  GAPBondMgr_SetParameter(GAPBOND_SECURE_CONNECTION, sizeof(uint8_t), &sc);
}
...
static void JustWorksScanner_logInsecureJustWorks(uint16_t connHandle, const char* reason)
{
  ...
  if (addrStr){
    for (uint8_t i=0;i<MAX_NUM_BLE_CONNS;i++)
      if (connList[i].connHandle == connHandle){ nm = lookupName(connList[i].addr); break; }
  }
...
{
  switch (t){
    case ADDRTYPE_PUBLIC:    return "PUBLIC";
    case ADDRTYPE_RANDOM:    return "RANDOM";
    case ADDRTYPE_PUBLIC_ID: return "PUBLIC_ID";
    case ADDRTYPE_RANDOM_ID: return "RANDOM_ID";
...
static const char* evtTypeToStr(uint16_t evtType, char* buf, size_t n)
{
  char tmp[64]; tmp[0]=0;
  bool first=true;
  #define ADD(bit,txt) do{ if (evtType & (bit)) { if(!first) strncat(tmp,"|",sizeof(tmp)-strlen(tmp)-1); strncat(tmp,(txt),sizeof(tmp)-strlen(tmp)-1); first=false; } }while(0)
  ...
  #undef ADD
  if (first) strncpy(tmp,"NONE",sizeof(tmp)-1);
  tmp[sizeof(tmp)-1]=0;
  if (n){ strncpy(buf,tmp,n-1); buf[n-1]=0; }
  return buf;
}
...
static const char* discReasonToStr(uint8_t r)
{
  switch (r)
  {
    ...
    case 0x02: return "UNKNOWN_CONNECTION";
    case 0x03: return "HW_FAILURE";
    case 0x05: return "AUTH_FAILURE";
    case 0x06: return "PIN_OR_KEY_MISSING";
    case 0x08: return "CONN_TIMEOUT";
    case 0x09: return "REPEATED_ATTEMPTS";
    case 0x0C: return "COMMAND_DISALLOWED";
    case 0x12: return "LL_RESPONSE_TIMEOUT";
    case 0x13: return "REMOTE_USER_TERM";
    case 0x14: return "REMOTE_LOW_RES";
    case 0x15: return "REMOTE_POWER_OFF";
    case 0x16: return "LOCAL_HOST_TERM";
    case 0x1A: return "UNSUPPORTED_FEATURE";
    case 0x28: return "PAIRING_NOT_ALLOWED";
    case 0x29: return "UNIT_KEY_UNSUPPORTED";
    case 0x2A: return "UNACCEPTABLE_CONN_INTERVAL";
    case 0x2F: return "MIC_FAILURE";
    case 0x30: return "CONN_FAILED_TO_ESTABLISH";
    case 0x3B: return "BAD_CONN_PARAMETERS";
...
static const char* pairStatusToStr(uint8_t s)
{
  switch (s) {
    case 0x00: return "SUCCESS";
    case 0x01: return "FAIL_PASSKEY_ENTRY";
    case 0x02: return "FAIL_OOB_NOT_AVAILABLE";
    case 0x03: return "AUTH_REQUIREMENTS";
    case 0x04: return "CONFIRM_VALUE_FAILED";
    case 0x05: return "AUTH_FAILURE";
    case 0x06: return "KEY_MISSING";
    case 0x07: return "ENCRYPTION_NOT_ACCEPTED";
    case 0x08: return "PAIRING_NOT_SUPPORTED";
    case 0x09: return "INSUFFICIENT_AUTHENTICATION";
    case 0x0A: return "CONFIRM_FAILED";
    case 0x0B: return "NOT_SUPPORTED";
    case 0x0C: return "ENCRYPTION_KEY_SIZE";
    case 0x0D: return "CMD_NOT_SUPPORTED";
    case 0x0E: return "UNSPECIFIED_REASON";
    case 0x0F: return "REPEATED_ATTEMPTS";
    case 0x10: return "INVALID_PARAMETERS";
    case 0x11: return "DHKEY_CHECK_FAILED";
    case 0x12: return "NUMERIC_COMPARISON_FAILED";
    case 0x13: return "BR_EDR_PAIRING_IN_PROGRESS";
    case 0x14: return "CTKD_NOT_ALLOWED";
    case 0x3E: return "CONN_FAILED_TO_ESTABLISH";
    default:   return "UNKNOWN";
  }
}
