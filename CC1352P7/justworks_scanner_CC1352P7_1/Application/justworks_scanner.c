...
#define JW_ADV_RPT_FIELDS                                                             \
    (SCAN_ADVRPT_FLD_EVENTTYPE | SCAN_ADVRPT_FLD_ADDRESS | SCAN_ADVRPT_FLD_ADDRTYPE | \
     SCAN_ADVRPT_FLD_PRIMPHY | SCAN_ADVRPT_FLD_SECPHY | SCAN_ADVRPT_FLD_DATALEN)

#define JW_ASSERT(expr) \
    if(!(expr)) JustWorksScanner_spin()
...
/* BondMgr callbacks */
static void JustWorksScanner_passcodeCb(
    uint8_t* deviceAddr,
    uint16_t connHandle,
    uint8_t uiInputs,
    uint8_t uiOutputs,
...
static void cacheNameRssi(uint8_t addr[B_ADDR_LEN], const char* name, int8 rssi) {
    ...
    for(i = 0; i < NAME_CACHE_MAX; i++) {
        if(memcmp(nameCache[i].addr, addr, B_ADDR_LEN) == 0) {
            if(name) {
                strncpy(nameCache[i].name, name, sizeof(nameCache[i].name) - 1);
                nameCache[i].name[sizeof(nameCache[i].name) - 1] = 0;
            }
    ...
    if(freeIdx != 0xFF) {
        ...
        if(name) {
            strncpy(nameCache[freeIdx].name, name, sizeof(nameCache[freeIdx].name) - 1);
            nameCache[freeIdx].name[sizeof(nameCache[freeIdx].name) - 1] = 0;
        } else
...
static int8 lookupRssi(uint8_t addr[B_ADDR_LEN]) {
    for(uint8_t i = 0; i < NAME_CACHE_MAX; i++)
        if(memcmp(nameCache[i].addr, addr, B_ADDR_LEN) == 0) return nameCache[i].lastRssi;
    return 0;
}
...
static void JustWorksScanner_taskFxn(uintptr_t a0, uintptr_t a1) {
    ...
    for(;;) {
        uint32_t events =
            Event_pend(syncEvent, Event_Id_NONE, JW_ALL_EVENTS, ICALL_TIMEOUT_FOREVER);
            ...
            if(ICall_fetchServiceMsg(&src, &dest, (void**)&pMsg) == ICALL_ERRNO_SUCCESS) {
                ...
                if((src == ICALL_SERVICE_CLASS_BLE) && (dest == selfEntity)) {
                    ...
                    if(pEvt->signature != 0xFFFF)
                        safeToDealloc = JustWorksScanner_processStackMsg((ICall_Hdr*)pMsg);
                }
...
static uint8_t JustWorksScanner_processStackMsg(ICall_Hdr* pMsg) {
    ...
    case HCI_GAP_EVENT_EVENT:
        if(pMsg->status == HCI_BLE_HARDWARE_ERROR_EVENT_CODE) DPRINTF("[DBG] HCI hardware error");
        break;
    default:
        break;
    }
...
static void JustWorksScanner_processAppMsg(jwEvt_t* pMsg) {
    ...
    case JW_EVT_ADV_REPORT: {
        ...
        if(logAllowed(pAdv->addr)) {
            ...
            DPRINTF(
                "[SCAN] ADV from %s | name:%s | RSSI:%d dBm | addrType:%s | primPHY:%s secPHY:%s evt:%s",
                Util_convertBdAddr2Str(pAdv->addr),
                name[0] ? name : "(no-name)",
                (int)pAdv->rssi,
                addrTypeStr(pAdv->addrType),
                phyToStr(pAdv->primPhy),
                phyToStr(pAdv->secPhy),
                evtStr);
        }
        ...
        if(numConn < MAX_NUM_BLE_CONNS && (pAdv->evtType & ADV_RPT_EVT_TYPE_CONNECTABLE)) {
            ...
            for(int i = 0; i < CONNECT_TRY_MAX; i++) {
                if(memcmp(connectTries[i].addr, pAdv->addr, B_ADDR_LEN) == 0 &&
            ...
            if(!exhausted && connectTry_allowedAndMark(pAdv->addr)) {
                ...
                GapInit_connect(
                    pAdv->addrType & MASK_ADDRTYPE_ID,
                    pAdv->addr,
                    DEFAULT_INIT_PHY,
                    CONNECTION_TIMEOUT);
            } else if(exhausted) {
                DPRINTF(
                    "[CONN] Max attempts reached for %s; skipping further tests",
                    Util_convertBdAddr2Str(pAdv->addr));
            }
        }
    ...
    case JW_EVT_SCAN_RESTART:
        ...
        case SCAN_STATE_DISABLING:
            wantRestart = true;
            DPRINTF(
                "[SCAN] transition in progress (%s); will restart after it completes",
    ...
    case JW_EVT_SCAN_WATCHDOG: {
        ...
        if((scanState == SCAN_STATE_ENABLING || scanState == SCAN_STATE_DISABLING) &&
           (now - lastScanTransitionMs) > SCAN_TRANSITION_TIMEOUT_MS) {
            DPRINTF(
                "[SCAN][WD] transition timeout in state=%d; forcing disable→enable",
                (int)scanState);
            ...
            if(status == SUCCESS) {
                ...
                if(idx < MAX_NUM_BLE_CONNS && !sawPasscodePrompt[idx])
                    JustWorksScanner_logInsecureJustWorks(
                        ch, "no user interaction during pairing");
            } else {
                DPRINTF(
                    "[PAIR] failed (0x%02X %s) conn=0x%04X", status, pairStatusToStr(status), ch);
            }
        } else if(pMsg->hdr.state == GAPBOND_PAIRING_STATE_ENCRYPTED) {
            DPRINTF(
                "[PAIR] %s (conn=0x%04X)",
                (status == SUCCESS) ? "link encrypted" : "encryption failed",
                ch);
        } else if(pMsg->hdr.state == GAPBOND_PAIRING_STATE_BOND_SAVED) {
            DPRINTF(
                "[PAIR] bond %s (conn=0x%04X)", (status == SUCCESS) ? "saved" : "save failed", ch);
        }
    } break;
    ...
    case JW_EVT_PASSCODE_NEEDED: {
        ...
        DPRINTF(
            "[PAIR] PasscodeReq conn=0x%04X uiIn:%u uiOut:%u numCmp:%lu",
            ch,
            pData->uiInputs,
            pData->uiOutputs,
...
static void JustWorksScanner_processGapMsg(gapEventHdr_t* pMsg) {
    ...
    case GAP_DEVICE_INIT_DONE_EVENT: {
        ...
        GapScan_setPhyParams(
            DEFAULT_SCAN_PHY, DEFAULT_SCAN_TYPE, DEFAULT_SCAN_INTERVAL, DEFAULT_SCAN_WINDOW);
        ...
        /* Initiator params */
        GapInit_setPhyParam(
            DEFAULT_INIT_PHY, INIT_PHYPARAM_CONN_INT_MIN, INIT_PHYPARAM_MIN_CONN_INT);
        GapInit_setPhyParam(
            DEFAULT_INIT_PHY, INIT_PHYPARAM_CONN_INT_MAX, INIT_PHYPARAM_MAX_CONN_INT);
        ...
        if(addrMode > ADDRMODE_RANDOM) {
            ...
            Util_constructClock(
                &clkRpaRead, JustWorksScanner_clockHandler, 30000, 0, true, JW_EVT_READ_RPA);
        }
    ...
    case GAP_LINK_ESTABLISHED_EVENT: {
        ...
        DPRINTF(
            "[CONN] Connected: %s | name:%s | RSSI:%d",
            Util_convertBdAddr2Str(e->devAddr),
            lookupName(e->devAddr),
            (int)lookupRssi(e->devAddr));
    ...
    case GAP_LINK_TERMINATED_EVENT: {
        ...
        DPRINTF(
            "[CONN] Disconnected: %s (reason=0x%02X %s)",
            Util_convertBdAddr2Str(connList[idx].addr),
            e->reason,
    ...
    case GAP_LINK_PARAM_UPDATE_EVENT: {
        ...
        if(linkDB_GetInfo(e->connectionHandle, &info) == SUCCESS)
            DPRINTF(
                "[CONN] Param update: %s timeout:%dms",
...
static void JustWorksScanner_processGATTMsg(gattMsgEvent_t* pMsg) {
    ...
    if(c) {
        ...
        case ATT_ERROR_RSP: {
            ...
            if(c->step == DISC_READ_DEVNAME) {
                DPRINTF(
                    "[INFO] GAP info flow stopped: DevName read failed (err=0x%02X, attr=0x%04X)",
                    e->errCode,
                    e->handle);
                c->step = DISC_DONE;
...
static nameDisc_t* getNameCtx(uint16_t connHandle) {
    for(uint8_t i = 0; i < MAX_NUM_BLE_CONNS; i++)
        if(nameCtx[i].inUse && nameCtx[i].connHandle == connHandle) return &nameCtx[i];
    return NULL;
}
...
static void startDevNameDiscovery(uint16_t connHandle) {
    ...
    bStatus_t s = GATT_DiscPrimaryServiceByUUID(
        connHandle, (uint8*)uuid_gap_1800, ATT_BT_UUID_SIZE, selfEntity);
    if(s != SUCCESS) {
...
static void JustWorksScanner_passcodeCb(
    ...
    if(pData) {
        ...
        if(JustWorksScanner_enqueueMsg(JW_EVT_PASSCODE_NEEDED, 0, (uint8_t*)pData) != SUCCESS)
            ICall_free(pData);
    }
}
...
static void JustWorksScanner_pairStateCb(uint16_t connHandle, uint8_t state, uint8_t status) {
    ...
    if(p) {
        ...
        p->status = status;
        if(JustWorksScanner_enqueueMsg(JW_EVT_PAIR_STATE, state, (uint8_t*)p) != SUCCESS)
            ICall_free(p);
    }
}
...
static void JustWorksScanner_logInsecureJustWorks(uint16_t connHandle, const char* reason) {
    ...
    DPRINTF(
        "[INSECURE] Just Works accepted by peer: addr=%s name=%s %s",
